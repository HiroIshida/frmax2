...
class PR2PybulletRobotInterface(PybulletRobotInterface):
    ...
    def wait_interpolation(self, thresh=0.05, timeout=60.0, callback=None):
        ...
        self.joint_name_to_joint_id["torso_lift_joint"]
        ...
        while True:
            ...
            for idx in self.joint_ids:
                ...
                if idx is None:
                    continue
                _, velocity, _, _ = pybullet.getJointState(self.robot_id, idx)
                if abs(velocity) > thresh:
                    joint_id_to_name[idx]
...
class Environment:
    ...
    def __init__(self, gui: bool = False):
        ...
        self.table_id = pybullet.createMultiBody(
            baseMass=0.0,
        ...
        pybullet.setTimeStep(1.0 / 1000.0)
    ...
    def solve_ik(self, co: Coordinates, simulate: bool = False, random_sampling: bool = False):
        ...
        if random_sampling:
            res = satisfy_by_optimization_with_budget(
                goal_eq_const, box_const, collfree_const, q_start
            )
        else:
    ...
    def translate(
        self, trans: np.ndarray, simulate: bool = False, random_sampling: bool = False
    ) -> bool:
